...
hist.plot <- function(traits, data, type, file, colourby, ...) {
  plotList <- list()
  for (i in traits) {
...
hist.plot(
  traits = traits17, data = pheno17, type = "Phenotypes2017",
  file = "./Figures/Distributions/", colourby = "location"
)
hist.plot(
...
hist.plot(
  traits = traits18, data = pheno18, type = "Phenotypes2018",
  file = "./Figures/Distributions/", colourby = "location"
)
hist.plot(
...
selectTop <- function(dat, traits, file, identifier, selecTrait, ...) {
  ...
  for (i in traits) {
    ...
    plot1 <- ggplot(
      ...
      geom_vline(
        xintercept = mean(topSelec$Trait),
    ...
    print(plots)
    ggexport(plots,
      filename = paste0(file, identifier, "_", i, ".png"),
...
top17d <- setDT(top17, keep.rownames = T)
top17d <- top17d %>%
  gather(
    key = "trait_id", value = "Variety",
    red_edge_2017_06_16:gndvi_2017_05_06
  ) %>%
  dplyr::group_by(Variety) %>%
  summarise(count = n())

top18d <- setDT(top18, keep.rownames = T)
top18d <- top18d %>%
  gather(
    key = "trait_id", value = "Variety",
    re_2018_06_11:canopy_height_2017_12_01
  ) %>%
  dplyr::group_by(Variety) %>%
  summarise(count = n())
