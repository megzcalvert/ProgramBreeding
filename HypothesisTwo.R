...
genoMaster <- genoMaster_2018 %>%
  bind_rows(
    genoMaster_2019, genoPYN_2019, genoGSPYN,
...
write.table(genoFile, "./GenoDatabase/BreedingProgramLines.txt",
  quote = F,
  sep = "\t", row.names = F, col.names = T
)
write.table(genoMaster, "./GenoDatabase/MasterBreedingProgram.txt",
  quote = F,
  sep = "\t", row.names = F, col.names = T
)
...
write.table(snpChip,
  file = "./GenoDatabase/SelectedImputedBeagle.txt",
...
write.table(snpChip,
  file = "./GenoDatabase/SelectedImputedBeagleNumeric.txt",
  col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE
)
...
Scores %>%
  ggplot(aes(x = PC1, y = PC2, colour = factor(Year))) +
  geom_point(alpha = 0.25) +
  stat_ellipse() +
  scale_color_manual(
    name = "Year",
    values = c(
      "#875692", "#f38400", "#be0032",
      "#008856", "#0067a5", "#604e97",
      "#b3446c", "#882d17", "#000000"
    )
  ) +
  scale_shape_manual(
    name = "DoubleHaploid",
  ...
  labs(
    title = "PCA of Breeding Program Markers",
